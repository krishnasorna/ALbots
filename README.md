...
The mutation of a Ludobot is what differentiates it from other Ludobots, and as shown above, the mutation can affect not only the brain, but also the body. There is a random mutation that changes the way a Ludobot interacts with its surroundings and that is where the brain of the Ludobot comes in. The brain seeks to either change its motor strength via the synapse values. Based on the value it is given, it will interact either with a greater strength of the motors or change the shape of the Ludobot. The physiological change of a Ludobot comes from the need to support a strong motor power or to provide stability. The Ludobot needs to be in a good shape as it has to traverse the simulation terrain towards the cube. It still needs a mutated body that either changes the size and positon of a linking cube. This is all done randomly via a selective mechanism and the Ludobot will store which body is the best one. The links can either get smaller or bigger or even completely removed from generation to generation. This is all based on the fitness function that we have defined and it will calibrate its body based on what parent or child has a better fitness and keep that Ludobot links the same. The generation of the links is all done via a random math function in Python that chooses values psuedo randomly. This helps to keep the Ludobot in a state of genetic mutations that support the brain's synapse weights as well as the motor strength that comes with.
